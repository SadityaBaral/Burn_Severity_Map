...
var pre_start = '2024-10-15'; 
var pre_end   = '2025-01-31';

var post_start = '2025-02-15'; 
var post_end   = '2025-05-15';
...
var dNBR = pre_fire.select('NBR').subtract(post_fire.select('NBR')).rename('dNBR');

var water_mask = pre_fire.select('NDWI').lt(0); 
var snow_mask = pre_fire.select('NDSI').lt(0.2).and(post_fire.select('NDSI').lt(0.2));
...
var final_mask = water_mask.and(snow_mask).and(slope_mask).and(veg_mask);
...
var burn_smooth = burn_classes.focal_mode({radius: 1.5, kernelType: 'square', iterations: 1});
var final_map = burn_smooth.updateMask(final_mask).clip(nepal);

var visParams = {min: 0, max: 3, palette: ['006400', 'FFFF00', 'FFA500', 'FF0000']};

Map.addLayer(pre_fire, {bands: ['B4', 'B3', 'B2'], min: 0, max: 0.3}, 'True Color (Late 2024)', false);
Map.addLayer(final_map, visParams, 'Burn Severity (2025)');
Map.addLayer(ee.Image().paint(nepal, 0, 2), {palette: 'black'}, 'Boundary');

var legend = ui.Panel({style: {position: 'bottom-left', padding: '8px 15px'}});
legend.add(ui.Label({value: 'Burn Severity (2025)', style: {fontWeight: 'bold', fontSize: '18px'}}));
...
legend.add(makeRow('006400', 'Unburned'));
legend.add(makeRow('FFFF00', 'Low Severity'));
legend.add(makeRow('FFA500', 'Moderate Severity'));
legend.add(makeRow('FF0000', 'High Severity'));
...
Export.image.toDrive({
  image: export_image,
  description: 'Nepal_Burn_Map_2025',
  scale: 30, 
  region: nepal,
  maxPixels: 1e13,
  crs: 'EPSG:4326',
